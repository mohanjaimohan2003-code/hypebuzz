...
-- Search is a read-only, security-invoker function. The caller's existing RLS
-- policies remain in force for every table referenced by the query.
...
as $$
  ...
  where product.status = 'published'
    and (
      ...
      or strpos(lower(brand.name), lower(btrim(p_query))) > 0
      or strpos(lower(category.name), lower(btrim(p_query))) > 0
      or exists (
        select 1
        from public.product_offers as matching_offer
        join public.merchants as merchant
          on merchant.id = matching_offer.merchant_id
    ...
    and (
      ...
      or exists (
        ...
        join public.merchants as merchant
          on merchant.id = merchant_offer.merchant_id
  ...
  order by
    ...
    case when p_sort in ('newest', 'popular') then product.created_at end desc,
    product.name asc
  limit least(greatest(coalesce(p_limit, 48), 1), 100);
$$;

revoke all on function public.search_products(text, text, text, text, numeric, numeric, numeric, text, boolean, text, integer) from public;
grant execute on function public.search_products(text, text, text, text, numeric, numeric, numeric, text, boolean, text, integer) to anon, authenticated;
