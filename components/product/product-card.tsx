    ...
    <article className="group flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white shadow-[0_2px_8px_rgba(17,24,39,0.05)] transition duration-200 hover:-translate-y-0.5 hover:border-[#BFDBFE] hover:shadow-[0_12px_24px_rgba(17,24,39,0.10)] motion-reduce:transform-none motion-reduce:transition-none">
      <div className="relative aspect-[16/10] overflow-hidden bg-white">
        <Link aria-label={`View ${product.name}`} className={`block h-full w-full ${focus}`} href={product.productHref}>
          {product.imageSrc.startsWith("/") ? <Image alt={product.imageAlt} className="object-contain p-3 transition-transform duration-200 group-hover:scale-[1.03] motion-reduce:transform-none" fill sizes="(max-width: 639px) 92vw, (max-width: 1023px) 46vw, 23vw" src={product.imageSrc} /> : (
            // Catalog images can come from approved merchant and media hosts.
            // eslint-disable-next-line @next/next/no-img-element
            <img alt={product.imageAlt} className="h-full w-full object-contain p-3 transition-transform duration-200 group-hover:scale-[1.03] motion-reduce:transform-none" loading="lazy" src={product.imageSrc} />
          )}
        </Link>
      ...
      <div className="flex flex-1 flex-col border-t border-[#F1F5F9] p-3.5">
        ...
        <div className="mt-2.5">{price ? <><p className="text-xs text-[#6B7280]">From</p><p className="text-lg font-bold leading-6 text-[#1455E8]">{price}</p><p className="text-xs text-[#6B7280]">Across {product.storeCount} {product.storeCount === 1 ? "store" : "stores"}</p></> : <><p className="text-sm font-semibold text-[#111827]">Price unavailable</p><p className="mt-1 text-xs text-[#6B7280]">Check the product page for updates.</p></>}</div>
        <div className="mt-auto pt-3"><CompareButton productId={product.id} productName={product.name}/></div>
      </div>
    </article>
  );
}
