...
export const ProductOffersField = forwardRef<ProductOffersFieldHandle, { initialOffers: ProductOfferValue[]; merchants: AdminMerchantOption[]; disabled: boolean; error?: string }>(function ProductOffersField({ initialOffers, merchants, disabled, error }, ref) {
        ...
        {offers.map((offer, index) => (
          <article className="rounded-xl border border-[#E5E7EB] p-4" key={offer.id}>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              <div><label className="text-sm font-semibold" htmlFor={`product-offer-merchant-${index}`}>Merchant</label><select className={inputClass} id={`product-offer-merchant-${index}`} onChange={(event) => update(index, "merchantId", event.target.value)} required value={offer.merchantId}><option value="">Select a merchant</option>{merchants.map((merchant) => <option disabled={!merchant.isActive && merchant.id !== offer.merchantId} key={merchant.id} value={merchant.id}>{merchant.name}{merchant.isActive ? "" : " (inactive)"}</option>)}</select></div>
              ...
              <div className="lg:col-span-2"><label className="text-sm font-semibold" htmlFor={`product-offer-url-${index}`}>Affiliate URL</label><input className={inputClass} id={`product-offer-url-${index}`} maxLength={2048} onChange={(event) => update(index, "affiliateUrl", event.target.value)} required type="url" value={offer.affiliateUrl} /></div>
              <div><label className="text-sm font-semibold" htmlFor={`product-offer-current-${index}`}>Current price</label><input className={inputClass} id={`product-offer-current-${index}`} min="0.01" onChange={(event) => update(index, "currentPrice", event.target.value ? Number(event.target.value) : null)} required step="0.01" type="number" value={offer.currentPrice ?? ""} /></div>
              <div><label className="text-sm font-semibold" htmlFor={`product-offer-original-${index}`}>Original price (optional)</label><input className={inputClass} id={`product-offer-original-${index}`} min="0.01" onChange={(event) => update(index, "originalPrice", event.target.value ? Number(event.target.value) : null)} step="0.01" type="number" value={offer.originalPrice ?? ""} /></div>
              <div><label className="text-sm font-semibold" htmlFor={`product-offer-currency-${index}`}>Currency</label><input className={inputClass} id={`product-offer-currency-${index}`} maxLength={3} onChange={(event) => update(index, "currency", event.target.value.toUpperCase())} required value={offer.currency} /></div>
              <div><label className="text-sm font-semibold" htmlFor={`product-offer-stock-${index}`}>Availability</label><select className={inputClass} id={`product-offer-stock-${index}`} onChange={(event) => update(index, "stockStatus", event.target.value as OfferStockStatus)} value={offer.stockStatus}><option value="in_stock">In stock</option><option value="limited_stock">Limited stock</option><option value="pre_order">Pre-order</option><option value="out_of_stock">Out of stock</option><option value="unknown">Unknown</option></select></div>
