    ...
    <>
      <div className="space-y-4 md:hidden">
        {brands.map((brand) => (
          <article key={brand.id} className="rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-[0_1px_2px_rgba(17,24,39,0.04)]">
            ...
            <dl className="mt-4 grid grid-cols-2 gap-4 border-t border-[#E5E7EB] pt-4 text-sm"><div><dt className="text-[#6B7280]">Products</dt><dd className="mt-1 font-semibold text-[#111827]">{brand.productCount}</dd></div><div><dt className="text-[#6B7280]">Updated</dt><dd className="mt-1 font-medium text-[#111827]"><time dateTime={brand.updated_at}>{formatDate(brand.updated_at)}</time></dd></div></dl>
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[68rem] border-collapse text-left">
          <caption className="sr-only">Brands sorted alphabetically by name</caption>
          <thead className="bg-[#F8FAFC] text-xs font-semibold uppercase tracking-wide text-[#6B7280]"><tr><th className="px-5 py-3" scope="col">Brand logo and name</th><th className="px-5 py-3" scope="col">Slug</th><th className="px-5 py-3" scope="col">Product count</th><th className="px-5 py-3" scope="col">Status</th><th className="px-5 py-3" scope="col">Updated</th><th className="px-5 py-3" scope="col">Actions</th></tr></thead>
          <tbody className="divide-y divide-[#E5E7EB]">{brands.map((brand) => <tr key={brand.id} className="align-middle"><th className="px-5 py-4" scope="row"><div className="flex items-center gap-3"><BrandLogoPreview logoUrl={brand.logo_url} name={brand.name} /><span className="max-w-xs truncate text-sm font-semibold text-[#111827]">{brand.name}</span></div></th><td className="px-5 py-4 text-sm text-[#6B7280]">/{brand.slug}</td><td className="px-5 py-4 text-sm font-semibold text-[#111827]">{brand.productCount}</td><td className="px-5 py-4"><BrandStatusBadge isActive={brand.is_active} /></td><td className="px-5 py-4 text-sm text-[#6B7280]"><time dateTime={brand.updated_at}>{formatDate(brand.updated_at)}</time></td><td className="px-5 py-4"><BrandActions brandId={brand.id} brandName={brand.name} isActive={brand.is_active} /></td></tr>)}</tbody>
        </table>
      </div>
    </>
  );
