...
export default async function EditCategoryPage({ params }: { params: Promise<{ categoryId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <CategoryForm category={{ id: category.id, name: category.name, slug: category.slug, description: category.description ?? "", iconUrl: category.image_url ?? "", isActive: category.is_active }} mode="edit" />
    </div>
  );
}
