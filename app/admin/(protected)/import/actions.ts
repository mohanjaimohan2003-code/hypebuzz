...
export async function importProductsCsv(_state:CsvImportState,formData:FormData):Promise<CsvImportState>{
  ...
  for(let index=0;index<rows.length;index++){
    ...
    if(match&&match.confidence>=95){const existed=match.product.merchantIds?.includes(merchant.id);const saved=await supabase.rpc("upsert_master_product_offers",{p_product_id:match.product.id,p_offers:[offer] as unknown as Json});results.push({row:index+2,product:label,action:saved.error?"error":existed?"updated":"attached",detail:saved.error?saved.error.message:`${match.confidence}% match: ${match.product.name}`});continue;}
