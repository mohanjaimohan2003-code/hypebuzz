...
export default async function EditOfferPage({ params }: { params: Promise<{ offerId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <OfferForm merchants={merchants} mode="edit" offer={{ id: offer.id, productId: offer.product_id, merchantId: offer.merchant_id, affiliateUrl: offer.affiliate_url, currentPrice: Number(offer.current_price), originalPrice: offer.original_price === null ? null : Number(offer.original_price), currency: offer.currency, stockStatus: offer.availability && isOfferStockStatus(offer.availability) ? offer.availability : "unknown", isActive: offer.is_active, notes: offer.coupon_note ?? "", shippingNote: offer.shipping_note ?? "", offerTitle: offer.offer_title ?? "", lastCheckedAt: offer.last_checked_at ? new Date(offer.last_checked_at).toISOString().slice(0, 16) : "" }} products={products} />
    </div>
  );
}
