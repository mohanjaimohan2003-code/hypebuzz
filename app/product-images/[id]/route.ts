...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if(!image?.storage_path)return new NextResponse("Not found",{status:404}); const result=await supabase.storage.from("product-images").download(image.storage_path); if(result.error){console.error({step:"download product image storage object",imageId:id,bucket:"product-images",objectPath:image.storage_path,code:result.error.name,message:result.error.message});return new NextResponse("Not found",{status:404});}
